...
export default function LanguagePage() {
  ...
  const [selected, setSelected] = useState<string[]>([]); // start with none
  ...
  return (
    <main className="space-y-6">
      <h1 className="text-2xl font-semibold">Language of Flowers</h1>
      <p className="text-sm text-neutral-500">Select meanings (love, gratitude, respect…) to filter.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
        {results.map((f) => (
          <FlowerCard f={f} key={f.id} href={`/encyclopedia/${f.id}?tab=language`} />
        ))}
      </div>
