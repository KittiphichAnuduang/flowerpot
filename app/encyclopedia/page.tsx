...
export default function EncyclopediaPage() {
  ...
  const results = useMemo(
    () => filterAndSort(all, query, selected), // filtering auto-enabled when tags selected
    [all, query, selected]
  );

  return (
    <main className="space-y-6">
      <h1 className="text-2xl font-semibold">Flower Encyclopedia</h1>
      <p className="text-sm text-neutral-500">Browse all flowers. Select tags to filter.</p>
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
        {results.map((f) => (
          <FlowerCard f={f} key={f.id} href={`/encyclopedia/${f.id}?tab=overview`} />
        ))}
      </div>
